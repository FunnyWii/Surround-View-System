...
std::pair<cv::Mat, cv::Mat> undist_by_remap(const cv::Mat& src, cv::Mat& dst, const CameraPrms& prms)
{
    ...
    if (!matrix_data || !scale || !shift) {
        // return;
        perror("!matrix_data || !scale || !shift , check matrix data! Err@common.cpp line 69...");
    }
    ...
    //undistort
    // cv::Mat map1, map2;
    std::pair<cv::Mat, cv::Mat> map;
    cv::fisheye::initUndistortRectifyMap(prms.camera_matrix, prms.dist_coff, cv::Mat(), new_camera_matrix, prms.size , CV_32FC1, map.first, map.second);
