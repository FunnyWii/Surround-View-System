...
static const  int shift_w = 200;
static const  int shift_h = 200;

static const  int cali_map_w  = 460;
static const  int cali_map_h  = 460;
//size of the gap between the calibration pattern and the car
//in horizontal and vertical directions
static const  int inn_shift_w = 80;
static const  int inn_shift_h = 55;
...
static const  int xl = shift_w + 100 + inn_shift_w;
static const  int xr = total_w - xl;
static const  int yt = shift_h + 100 + inn_shift_h;
...
static std::map<std::string, std::vector<cv::Point2f>> project_keypoints = {
    {"front", {cv::Point2f(shift_w + 100, shift_h),
              cv::Point2f(shift_w + 360, shift_h),
              cv::Point2f(shift_w + 100, shift_h + 100),
              cv::Point2f(shift_w + 360, shift_h + 100)}},

    {"back", {cv::Point2f(shift_w + 100, shift_h),
              cv::Point2f(shift_w + 360, shift_h),
              cv::Point2f(shift_w + 100, shift_h + 100),
              cv::Point2f(shift_w + 360, shift_h + 100)}},

    {"left", {cv::Point2f(shift_h + 100, shift_w),
              cv::Point2f(shift_h + 360, shift_w),
              cv::Point2f(shift_h + 100, shift_w + 100),
              cv::Point2f(shift_h + 360, shift_w + 100)}},

    {"right", {cv::Point2f(shift_h + 100, shift_w),
              cv::Point2f(shift_h + 360, shift_w),
              cv::Point2f(shift_h + 100, shift_w + 100),
              cv::Point2f(shift_h + 360, shift_w + 100)}}
};
